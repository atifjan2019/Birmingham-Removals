  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-slate-50 pt-20 pb-20">
        <section className="bg-gradient-to-br from-[#0A2540] to-[#14375C] text-white py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="bg-white rounded-2xl p-7 border border-slate-200">
              <h2 className="font-[family-name:var(--font-space)] font-bold text-lg text-[#0A2540] mb-5 flex items-center gap-2">
                <Navigation className="w-5 h-5 text-[#FF6B35]" /> Main Pages
              </h2>
              <ul className="space-y-2">
                {mainPages.map((p) => (
                  <li key={p.href}>
                    <Link href={p.href} className="text-slate-700 hover:text-[#FF6B35] text-sm">
                      {p.label}
                    </Link>
                  </li>
                ))}
            ...
            <div className="bg-white rounded-2xl p-7 border border-slate-200">
              <h2 className="font-[family-name:var(--font-space)] font-bold text-lg text-[#0A2540] mb-5 flex items-center gap-2">
                <Briefcase className="w-5 h-5 text-[#FF6B35]" /> Services
              </h2>
              <ul className="space-y-2">
                {servicesData.map((s) => (
                  <li key={s.slug}>
                    <Link href={`/services/${s.slug}`} className="text-slate-700 hover:text-[#FF6B35] text-sm">
                      {s.title}
                    </Link>
                  </li>
                ))}
            ...
            <div className="bg-white rounded-2xl p-7 border border-slate-200">
              <h2 className="font-[family-name:var(--font-space)] font-bold text-lg text-[#0A2540] mb-5 flex items-center gap-2">
                <Map className="w-5 h-5 text-[#FF6B35]" /> Coverage Areas
              </h2>
              <ul className="grid grid-cols-2 gap-2 text-sm">
                {areasList.map((a) => (
                  <li key={a.id}>
                    <Link href={`/areas/${a.id}`} className="text-slate-700 hover:text-[#FF6B35] flex items-center gap-1.5">
